...
const DoctorChat: React.FC<DoctorChatProps> = ({ language = "hindi", onSend }) => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), text: "कुछ गड़बड़ हो गई। कृपया दोबारा कोशिश करें।", sender: "doctor" },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <button
        onClick={() => setOpen((o) => !o)}
        className="fixed bottom-5 left-5 z-50 rounded-full px-5 py-3 font-bold text-sm shadow-lg transition-transform hover:scale-105 active:scale-95"
        ...
        {open && (
          <>
            ...
            >
              <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "#334155" }}>
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-lg" style={{ color: "#FF9933" }}>Dr. Raahat</span>
                  </div>
                  <p className="text-xs mt-0.5" style={{ color: "#94a3b8" }}>Aapka AI Doctor</p>
                </div>
                <button
                  onClick={() => setOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                {messages.length === 0 && !loading && (
                  <div className="flex flex-col gap-2 mt-8">
                    <p className="text-center text-sm mb-2" style={{ color: "#94a3b8" }}>
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                    <div
                      ...
                      style={
                        msg.sender === "user"
                          ? { backgroundColor: "#1e293b", borderLeft: "3px solid #FF9933", color: "#e2e8f0" }
              ...
              <div className="px-3 py-3 flex items-center gap-2 border-t" style={{ borderColor: "#334155" }}>
                <button
                ...
                <input
                  ...
                  style={{ backgroundColor: "#1e293b", color: "#e2e8f0", border: "2px solid transparent" }}
