...
// ============================================================
// exercise/page.tsx — MEMBER 4 OWNS THIS
// Adaptive weekly exercise plan from GUC exercise_flags
// Calls GET /exercise with flags from GUC
// ============================================================
...
const TIER_CONFIG: Record<
  string,
  { color: string; bg: string; icon: string; badge: string }
> = {
  LIGHT_WALKING_ONLY: {
    color: "#22C55E",
    bg: "#22C55E15",
    icon: "🚶",
    badge: "Light Recovery",
  },
  CARDIO_RESTRICTED: {
    color: "#06B6D4",
    bg: "#06B6D415",
...
export default function ExercisePage() {
  const { exerciseLevel, latestReport, profile, addXP, setAvatarState } =
    useGUCStore();
  ...
  const weekTotal = (data?.weekly_plan ?? []).reduce(
  ...
  return (
    <div className="min-h-screen bg-[#0d0d1a] text-white">
      ...
      <div
        ...
        style={{
          backgroundImage: `radial-gradient(circle at 70% 20%, ${tierCfg.color} 0%, transparent 50%)`,
        }}
      />
      ...
      <div className="relative max-w-2xl mx-auto px-4 py-6 pb-24">
            ...
            return (
              ...
              >
                ...
                <span className="text-base">
                  {isDone ? "✅" : isRest ? "💤" : tierCfg.icon}
                </span>
                <span
                  className="text-[9px] font-medium"
                  style={{ color: isSelected ? tierCfg.color : "rgba(255,255,255,0.3)" }}
                >
              ...
              {(() => {
                ...
                return (
                  <>
                    {/* Day header */}
                    <div
                      className="px-4 py-3 border-b border-white/[0.06] flex justify-between items-center"
                    >
                      <div>
                      ...
                      {day.duration_minutes === 0 ? (
                        <div className="text-center py-4">
                          ...
                          <p className="text-white/40 text-sm mt-2">
                            Full rest day. Your body repairs and grows stronger
                            while you rest.
                          </p>
                        </div>
                      ) : (
        ...
        {(data?.avoid ?? []).length > 0 && (
          ...
          >
            <p className="text-red-400/80 text-[10px] uppercase tracking-widest mb-2">
              ⚠️ Avoid
            </p>
...
// ── Hardcoded fallback (always renders, no API needed) ────────

const FALLBACK_PLAN: ExerciseResponse = {
  tier: "NORMAL_ACTIVITY",
  tier_description:
    "Standard moderate activity plan. 30-minute sessions, 5 days a week.",
  general_advice:
    "Stay consistent. 5 days of 30 minutes is better than 1 day of 2 hours. Drink water before and after exercise.",
  avoid: [
