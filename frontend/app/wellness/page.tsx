...
// ============================================================
// wellness/page.tsx — MEMBER 4 OWNS THIS
// Mood check-in + sleep tracker + breathing widget
// All data goes to GUC.mentalWellness
// ============================================================
...
const MOOD_CHART_TOOLTIP = ({ active, payload }: any) => {
...
export default function WellnessPage() {
  const { mentalWellness, profile, latestReport } = useGUCStore();
  ...
  const avgStress = mentalWellness.moodHistory.length
    ? Math.round(
        mentalWellness.moodHistory.slice(-7).reduce((s, e) => s + e.stress, 0) /
          Math.min(7, mentalWellness.moodHistory.length)
      )
    : mentalWellness.stressLevel;

  const avgSleep = mentalWellness.moodHistory.length
    ? (
        mentalWellness.moodHistory.slice(-7).reduce((s, e) => s + e.sleep, 0) /
        Math.min(7, mentalWellness.moodHistory.length)
      ).toFixed(1)
    : mentalWellness.sleepQuality;
      ...
      <div className="relative max-w-2xl mx-auto px-4 py-6 pb-24">
        ...
        >
          {[
            ...
            {
              label: "Streak",
              value: `${mentalWellness.moodHistory.length}d`,
              color: "#6366F1",
              icon: "🔥",
            },
          ].map((stat) => (
            <div
              key={stat.label}
              className="flex-1 bg-white/[0.04] border border-white/[0.07] rounded-xl py-2.5 px-3 text-center"
            >
        ...
        {/* ── Report-linked mental health note ───────────── */}
        {latestReport && mentalWellness.stressLevel <= 4 && (
          ...
          >
            <p className="text-orange-300/70 text-xs leading-relaxed">
              ...
              . Medical stress is real. Talking to Dr. Raahat in the chat, even
              for 5 minutes, can help reduce anxiety about your results.
            </p>
          </motion.div>
        )}
              ...
              <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-4">
                ...
                <div className="space-y-2.5">
                  {[
                    {
                      icon: "🩸",
                      text: "Iron is absorbed better when you sleep 7+ hours",
                    },
                    {
                      icon: "🦴",
                      text: "Bone repair and Vitamin D activation happen during deep sleep",
                    },
                    {
                      icon: "🛡️",
                      text: "Immune system strengthens during sleep cycles",
                    },
                    {
                      icon: "🧠",
                      text: "Stress hormones drop by 30% after a full night of sleep",
                    },
                  ].map((tip, i) => (
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex gap-3 text-[10px] text-white/30">
                      <span className="flex items-center gap-1">
                        <span className="w-2 h-0.5 bg-[#FF9933] inline-block rounded" />
                        Stress
                      </span>
                      <span className="flex items-center gap-1">
                        <span className="w-2 h-0.5 bg-[#22C55E] inline-block rounded" />
                        Sleep
                      </span>
                    </div>
                  </div>
                  <ResponsiveContainer width="100%" height={160}>
                    <LineChart data={moodChartData}>
                      <XAxis
                        ...
                        tickLine={false}
                      />
                      <Tooltip content={<MOOD_CHART_TOOLTIP />} />
                      <Line
                        type="monotone"
                        dataKey="stress"
                        stroke="#FF9933"
